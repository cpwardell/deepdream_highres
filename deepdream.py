...
graph_def = tf.GraphDef.FromString(open(FLAGS.model,"rb").read()) # SPECIFY BINARY
...
def tffunc(*argtypes):
    ...
    '''
    #placeholders = map(tf.placeholder, argtypes) # ORIGINAL
    placeholders = list(map(tf.placeholder, argtypes)) ## needs to be a list
    def wrap(f):
        out = f(*placeholders)
        def wrapper(*args, **kw):
